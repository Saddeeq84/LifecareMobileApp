        ...
        children: [
          ...
          Expanded(
            child: TabBarView(
              controller: _tabController,
              children: [
                DoctorBroadcastMessagesTab(currentUserId: currentUserId),
                DoctorCHWMessagesTab(currentUserId: currentUserId),
                DoctorPatientMessagesTab(currentUserId: currentUserId),
                DoctorFacilityMessagesTab(currentUserId: currentUserId),
              ],
            ),
          ),
        ],
...
// Doctor Broadcast Messages Tab
class DoctorBroadcastMessagesTab extends StatelessWidget {
  final String currentUserId;
  
  const DoctorBroadcastMessagesTab({super.key, required this.currentUserId});
  ...
  void _openMessage(BuildContext context, String messageId, Map<String, dynamic> data) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DoctorMessageDetailScreen(
          messageId: messageId,
          messageData: data,
...
// Doctor CHW Messages Tab
class DoctorCHWMessagesTab extends StatelessWidget {
  final String currentUserId;
  
  const DoctorCHWMessagesTab({super.key, required this.currentUserId});
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                const Text(
                  'Messages from Community Health Workers will appear here',
                  style: TextStyle(color: Colors.grey),
                  textAlign: TextAlign.center,
                ),
                const SizedBox(height: 16),
  ...
  void _openConversation(BuildContext context, String conversationId, Map<String, dynamic> data) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DoctorConversationScreen(
...
// Doctor Patient Messages Tab
class DoctorPatientMessagesTab extends StatelessWidget {
  final String currentUserId;
  
  const DoctorPatientMessagesTab({super.key, required this.currentUserId});
...
// Doctor Facility Messages Tab
class DoctorFacilityMessagesTab extends StatelessWidget {
  final String currentUserId;
  
  const DoctorFacilityMessagesTab({super.key, required this.currentUserId});
...
// Doctor Message Detail Screen for broadcast messages
class DoctorMessageDetailScreen extends StatefulWidget {
  final String messageId;
  final Map<String, dynamic> messageData;
  final String currentUserId;

  const DoctorMessageDetailScreen({
    super.key,
  ...
  @override
  State<DoctorMessageDetailScreen> createState() => _DoctorMessageDetailScreenState();
}

class _DoctorMessageDetailScreenState extends State<DoctorMessageDetailScreen> {
                                    ...
                                    Row(
                                      children: [
                                        CircleAvatar(
                                          ...
                                          child: const Icon(Icons.local_hospital, size: 16, color: Colors.indigo),
                                        ),
                                        const SizedBox(width: 8),
                                        Text(
                                          replyData['senderName'] ?? 'Doctor',
  ...
  void _sendReply() {
    ...
    FirebaseFirestore.instance
      ...
      'senderName': 'Doctor User', // You might want to get this from user profile
...
// Doctor Conversation Screen for direct messages
class DoctorConversationScreen extends StatefulWidget {
  ...
  @override
  State<DoctorConversationScreen> createState() => _DoctorConversationScreenState();
}
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Align(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: isCurrentUser ? Colors.indigo : Colors.grey.shade200,
